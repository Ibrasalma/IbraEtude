        ...
        <div class="panel-heading">
          <div class="row">
            <div class="col col-xs-6">
              <h3 class="panel-title">Il y'a {{ $photo->count() }}{{ Str::plural(' lettre de recommandation',$photo->count())}} documents soumis</h3>
            </div>
          </div>
        </div>
            ...
            <tbody>
              ...
              <p data-placement="top" data-toggle="tooltip" title="Delete">
                <form action="{{ route('avatars.destroy',$avatar) }}" method="POST" class="inline-block" onsubmit="return confirm('Êtes-vous sûr de vouloir supprimer  :{{ $avatar->id }}?');">
                  ...
                  <td>
                    <?php 
                      $image = App\Models\Letter1::where('id',$avatar->id)->first()->location;
